...
# 检查日志文件
if [ -f "storage/logs/service.log" ]; then
    echo "✓ 服务日志文件存在"
    echo "  日志文件大小: $(du -h storage/logs/service.log | cut -f1)"
    echo "  最后修改时间: $(stat -c %y storage/logs/service.log 2>/dev/null || stat -f %Sm storage/logs/service.log 2>/dev/null)"
    
    # 显示最近的日志
    echo "  最近日志 (最后10行):"
    echo "  ----------------------------------------"
    tail -10 storage/logs/service.log | sed 's/^/  /'
    echo "  ----------------------------------------"
else
    echo "✗ 服务日志文件不存在"
fi
