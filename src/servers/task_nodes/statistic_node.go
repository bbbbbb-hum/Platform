...
// Init 初始化空节点
func (s *StatisticNode) Init(config types.InitConfig) error {
	...
	logger.Info("统计节点初始化完成，不提供任何工具")
	return nil
}

// GetTools 获取工具列表 - 统计节点不提供任何工具
func (s *StatisticNode) GetTools(rc *types.RunningContext) (currentToolList []*types.ToolDesc) {
	// 统计节点不添加任何工具
	return
}

// Process 处理工具调用 - 空节点什么都不做
func (s *StatisticNode) Process(rc *types.RunningContext, userCmd string, userParamMap map[string]interface{}, lastStepResp *mcp.CallToolResult) (currentResp *mcp.CallToolResult, err error) {
	// 获取上一步的结果
	if lastStepResp != nil {
...
// GetNodeInfo 获取节点信息
func (s *StatisticNode) GetNodeInfo() *types.NodeInfo {
	return s.NodeInfo
}
