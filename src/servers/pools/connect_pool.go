...
func (c *ConnectionPool) InitializeNode(nodeID int32, protocol, nodeURL string, timeoutMS int) error {
	...
	if len(service.InstanceMap) == 0 {
		...
		return fmt.Errorf("list_tools_failed: no_instance_created node_id=%d server_id=%s protocol=%s", nodeID, serviceID, protocol)
	}
	firstInstance := firstConnectedInstance(service)
	if firstInstance == nil {
		closeNodeServiceSessions(service)
		return fmt.Errorf("list_tools_failed: no_active_connection node_id=%d server_id=%s protocol=%s", nodeID, serviceID, protocol)
	}
...
func firstConnectedInstance(service *ExternalService) *ServiceInstance {
	for _, instance := range service.InstanceMap {
		if instance != nil && len(instance.Connections) > 0 && instance.Connections[0] != nil && instance.Connections[0].Session != nil {
			return instance
		}
	}
	return nil
